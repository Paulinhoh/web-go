...
func (nh *noteHandler) NoteList(w http.ResponseWriter, r *http.Request) error {
	...
	if err != nil {
		return apperror.WithStatus(errors.New("aconteceu um erro ao executar essa página"), http.StatusMethodNotAllowed)
	}

	slog.Info("Execultou o handler /")

	if err := t.ExecuteTemplate(w, "base", nil); err != nil {
		return apperror.WithStatus(errors.New("falha ao carregar o template"), http.StatusNotFound)
	}
...
func (nh *noteHandler) NoteView(w http.ResponseWriter, r *http.Request) error {
	...
	if id == "0" {
		return apperror.WithStatus(errors.New("nota não encontrada"), http.StatusNotFound)
	}
